// v19: one NanumBarunGothic layout drives the canvas glyph, dotted guide and hit paths.
...
(() => {
  ...
  function fittedPaths(){
    ...
    const m=glyphLayout(cur()[0],r.width,r.height);
    const pts=raw.flat();
  ...
  function paintCalibratedGuide(){
    ...
    g.globalAlpha=.12;
    g.fillStyle='#8f857c';
    g.font=`400 ${m.size}px NBG`;
    g.textAlign='center';g.textBaseline='middle';
    g.fillText(cur()[0],r.width/2,m.baseline);
    g.restore();

    const ps=paths();
    dotsCanvas.width=Math.max(1,Math.round(r.width));
    dotsCanvas.height=Math.max(1,Math.round(r.height));
    dotsCtx.clearRect(0,0,r.width,r.height);
    ps.forEach((st,si)=>{
      const active=si===currentStroke&&!completed;
      dotsCtx.strokeStyle=active?'#8f8479':'#d2c8be';
      dotsCtx.lineWidth=Math.max(11,r.width*.032);
      dotsCtx.lineCap='round';dotsCtx.lineJoin='round';dotsCtx.setLineDash([2,14]);
      dotsCtx.beginPath();
      st.forEach((p,i)=>{const q=px(p,r);i?dotsCtx.lineTo(q.x,q.y):dotsCtx.moveTo(q.x,q.y)});
      dotsCtx.stroke();dotsCtx.setLineDash([]);
    });

    // Keep every visible dot inside the exact same glyph used for the background.
    dotsCtx.save();
    dotsCtx.globalCompositeOperation='destination-in';
    dotsCtx.fillStyle='#000';
    dotsCtx.font=`400 ${m.size}px NBG`;
    dotsCtx.textAlign='center';dotsCtx.textBaseline='middle';
    dotsCtx.fillText(cur()[0],r.width/2,m.baseline);
    dotsCtx.restore();
    g.drawImage(dotsCanvas,0,0,r.width,r.height);
  ...
  async function apply(){
    try{await document.fonts.load('400 80px NBG',cur()[0]);}catch(_){ }
